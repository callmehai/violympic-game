  ...
  return (
    <div className="min-h-screen bg-treasure-bg text-white">
      <header className="flex flex-wrap items-center justify-between gap-3 px-6 py-4">
        <h1 className="font-display text-2xl font-extrabold text-treasure-gold">⛰️ Vượt Ải Trí Tuệ</h1>
      ...
      <main className="mx-auto w-full max-w-3xl px-4 pb-12">
        ...
        {loading ? (
          <div className="panel animate-pop p-6 text-center text-white/60">Đang tải…</div>
    ...
    <div className="panel animate-pop space-y-6 p-6">
      <div className="text-center">
        ...
        <p className="mt-1 text-sm text-white/60">
          Điều khiển nhân vật tìm đường tới kho báu 💎. Qua <b className="text-amber-300">cửa 🚪/bẫy</b> phải trả lời — sai thì cửa khoá, đổi tuyến!
        </p>
      </div>

      {/* Thời gian + cửa + thưởng */}
      <div className="grid grid-cols-3 gap-2">
        <Stat icon="⏱️" big={fmtMinutes(timeLimitS)} label="Thời gian" tone="text-treasure-gold" />
        <Stat icon="🚪" big="9" label="Số cửa hỏi" tone="text-amber-300" />
        <Stat icon="💎" big={`+${finishMax.toLocaleString('vi-VN')}`} label="Thưởng đích" tone="text-emerald-300" />
      </div>

      {/* 4 kiểu câu */}
      <section className="space-y-3">
        <h3 className="font-bold text-treasure-gem">🧩 4 kiểu câu hỏi (dễ → khó dần)</h3>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {TYPES.map((t) => (
            <div key={t.name} className="rounded-xl border border-white/10 bg-black/30 p-3 text-center">
              <div className="text-2xl">{t.icon}</div>
              <div className="mt-1 text-sm font-bold text-white">{t.name}</div>
      ...
      {/* Điểm */}
      <section className="space-y-2 rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-white/80">
        <div>
          🚶 Đi bằng <b className="text-white">phím mũi tên</b> hoặc <b className="text-white">bấm ô kề</b> bên. Né 🪨, tìm 💎.
        </div>
        <div>
          🎯 Qua <b className="text-amber-300">cửa/bẫy</b> (số trên cửa = độ khó) → trả lời. <b className="text-white">Đúng</b> = mở đường + điểm theo độ khó (<b className="text-emerald-300">1 Dễ</b> +{dp.easy} · <b className="text-amber-300">2 TB</b> +{dp.medium} · <b className="text-rose-300">3 Khó</b> +{dp.hard}).
        </div>
        <div>
          ⚡ Trả lời càng nhanh càng được thưởng (tối đa{' '}
          <b className="text-treasure-gem">+{speedMax}đ</b>, giảm dần về 0 trong{' '}
          <b className="text-white">{fastSec} giây</b>).
        </div>
        <div>
          ❌ <b className="text-white">Sai</b>: cửa đó thành đá → phải đi tuyến khác (không mất mạng). Chỉ{' '}
          <b className="text-white">DỪNG khi không còn đường nào</b> tới kho báu.
        </div>
        <div>
          💎 Về tới kho báu = <b className="text-emerald-300">thưởng đích</b> = {finishBase} + (%giờ còn × {finishTime.toLocaleString('vi-VN')}) — tối đa ~{finishMax.toLocaleString('vi-VN')}đ.
        </div>
        <div className="rounded-xl border border-rose-400/50 bg-rose-500/10 px-3 py-2.5 text-sm font-bold text-white">
          ⏳ Với <span className="text-rose-300">mỗi giây bạn chưa về đích = đang mất ~{lossText} điểm</span> thưởng → trả lời gọn rồi nhanh chân về đích!
        </div>
        <div>
          ⏱️ Thời gian: <b className="text-white">{fmtMinutes(timeLimitS)}</b>. Xếp hạng theo <b className="text-white">điểm</b>, bằng điểm thì <b className="text-white">nhanh hơn</b> thắng.
        </div>
...
function ContinueCard({ onContinue }: { onContinue: () => void }) {
  return (
    <div className="panel animate-pop p-6 text-center">
      ...
      <p className="mb-5 mt-1 text-white/70">Bạn còn một lượt đi chưa về đích. Quay lại tiếp tục!</p>
    ...
    <div className="panel animate-pop p-6 text-center">
      ...
      <div className="my-5 grid grid-cols-3 gap-3">
        ...
        <div className="rounded-xl bg-black/30 p-3">
          ...
          <div className="text-xs text-white/50">Cổng mở</div>
        </div>
        <div className="rounded-xl bg-black/30 p-3">
